...
class CentralModelCommunication (LocalModel):
	...
	lock = RLock()
	# initialize 
	def __init__(self, data = "none", target_name = "AFclass", model_name = "dt4",random_state = 12345678, imputation_strategy = 'mice',balance_strategy = 'SMOTE', local_models = []):
			...
			try:
				...
				try :
					...
					if data[0] == "train":
						...
						if 'prediction' in data_received.columns:
							print ("qwer 1")
							self.trainDataPredictedChunk['prediction ' +self.clients[client]]= data_received['prediction']
							print ("qwer 2")
						print ("qwer 3")
						countResponses += 1
						print ("qwer 4")
	...
	def train_next_chunk(self):
		...
		# update trainset 
		print ("wtf 1")
		self.trainDataPredicted = pd.concat([self.trainDataPredicted,pd.DataFrame(self.trainDataPredictedChunk)]) # data (predicted) returned from the clients -> used to train the central model
		print ("wtf 2")
