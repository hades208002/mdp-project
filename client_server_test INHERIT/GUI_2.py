...
class GUI(LocalModelCommunication):
	def __init__(self):
		# overall
		LocalModelCommunication.__init__(self, data="none", target_name="AFclass", model_name="meiyou",
										 random_state=12345678, imputation_strategy='mice', balance_strategy='SMOTE',
										 local_models=[])
		...
		self.model_label_text.set('Press button to start training.')
		...
		self.faild_gif = PhotoImage(file='faild.GIF')
		self.connected_gif = PhotoImage(file='connected_2.GIF')

	# 5 step


	def connectionBegun(self):
		print("I'm the GUI and i'm connected!!!")
		self.connect_img.config(image=self.connected_gif)
		self.connect_img.pack()

	def connectionFailed(self):
		print ("connection failed")
		self.connect_img.config(image=self.faild_gif)
		self.connect_img.pack()
	...
	def start_training(self):
		self.addData(data=pd.DataFrame(self.dataframe))
		self.cv_result = self.chooseModel_with_crossValidation_and_train()
	...
	def tab_match(self, root):
		...
		column_head.pack(side=TOP)
		self.list_right = Listbox(frame, listvariable=self.rightbox, height=20, width=20)
		...
		column_head.pack(side=TOP)
		self.list_rright = Listbox(frame, listvariable=self.rrightbox, height=20, width=20)
	...
	def tab_model(self, root):
		...
		self.label_img = ttk.Label(self.label_frame)
		#self.label_img.pack()
		frame = Frame(root)
		frame.pack(side=BOTTOM)
		...
		print ("current dataframe? -> self.dataframe")
		print (self.dataframe)

		self.model_label_text.set('Wait for model training...')
		self.label_img.config( image=self.img_wait)
		self.label_img.pack()

		root.after(1000, self.start_training)


	def trainingdone(self):
		self.model_label_text.set('Training done.')
		ax = self.cv_result.plot.bar(x='Model', y='Accuracy', rot=45, figsize=(4, 3.3), title="Local Models' Accuracy")
		plt.pyplot.savefig('img.png')
		self.img_gif = ImageTk.PhotoImage(Image.open('img.png'))
	...
	def tab_connect(self, root):
		...
		self.connect_img = ttk.Label(label_frame, image=self.img_wait)
		self.connect_img.pack()
...
"""
### Modified ###
	trainingdone():	figsize, title
	tab_matche(): listbox width
